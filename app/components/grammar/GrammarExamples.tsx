  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6 sm:p-8 dark:border-neutral-800 dark:bg-neutral-900">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <AddExampleButton itemId={itemId} />
      </div>
      {examples.length === 0 ? (
...
function AddExampleButton({ itemId }: { itemId: number }) {
  ...
  async function generate() {
    ...
    setError(null);
    try {
      const data = await addGrammarExample(itemId, "default");
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        onClick={generate}
        title={ai.hasAi ? "AI 로 예문 1개 추가" : "AI 키 미설정"}
      ...
      >
        {busy ? <Spinner className="h-3 w-3" /> : "✦"} 예문 추가
      </button>
    </div>
  );
}
